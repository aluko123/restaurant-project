...
type PurchaseLine = { id: string; position: number; sku: string | null; description: string; quantity: string | null; unit: string | null; unitPrice: string | null; lineTotal: string | null; canTrack: boolean; suggestedAction: "match" | "ignore" | null; suggestedInventoryItemId: string | null; suggestedConversion: string | null };
...
type PurchaseDecision = { action: "" } | { action: "ignore"; suggested?: boolean } | { action: "match"; inventoryItemId: string; expectedCountUnit: string; conversion: string; suggested: boolean } | { action: "create"; name: string; category: string; countUnit: string; conversion: string };
type MenuItem = { id: string; name: string; category: string | null; sellingPrice: string; currency: string; active: boolean; ingredientCount: number; setupChoice: "important" | "later" | null; costState: "recipe_not_configured" | "purchase_cost_missing" | "partial" | "complete" };
type MenuImport = { id: string; originalFilename: string; status: string; delayed: boolean; lastError: string | null; createdAt: string };
...
function MenuWorkspace({ restaurant, request, active }: { restaurant: Restaurant; request: <T>(path: string, init?: RequestInit) => Promise<T>; active: boolean }) {
  ...
  const [choiceSavingId, setChoiceSavingId] = useState("");
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setError("");setNotice("");if(!name.trim()||!price.trim()){setError("Add the menu item name and selling price.");return}setSaving(true);try{const item=await request<MenuItem>("/v1/menu-items",{method:"POST",body:JSON.stringify({name,category:category||null,sellingPrice:price,currency})});setItems((current)=>[...current,item].sort((a,b)=>(a.category??"zzz").localeCompare(b.category??"zzz")||a.name.localeCompare(b.name)));setName("");setCategory("");setPrice("");setCreatedItem(item);setNotice(`${item.name} added.`);}catch(reason){setError(reason instanceof Error?reason.message:"The menu item couldn't be saved.");}finally{setSaving(false)}}
  async function chooseSetup(item:MenuItem,choice:"important"|"later"){setChoiceSavingId(item.id);setError("");try{await request(`/v1/menu-items/${item.id}/ingredient-setup-choice`,{method:"PUT",body:JSON.stringify({choice})});const updated={...item,setupChoice:choice};setItems(current=>current.map(value=>value.id===item.id?updated:value));setCreatedItem(null);if(choice==="important")setCostingItem(updated);else setNotice("Saved for later.");}catch(reason){setError(reason instanceof Error?reason.message:"Choice couldn't be saved.")}finally{setChoiceSavingId("")}}
  ...
  return <section className="menu-workspace" aria-labelledby="menu-heading">
    ...
    {createdItem&&<section className="menu-create-next" aria-labelledby="menu-create-next-heading"><h2 id="menu-create-next-heading">Add ingredients for {createdItem.name}?</h2><div><button className="ledger-button" type="button" disabled={choiceSavingId===createdItem.id} onClick={()=>void chooseSetup(createdItem,"important")}>Add ingredients</button><button className="file-button" type="button" disabled={choiceSavingId===createdItem.id} onClick={()=>void chooseSetup(createdItem,"later")}>Later</button></div></section>}
    <div className={`menu-grid${canManageMenu?"":" menu-grid-read-only"}`}>
      ...
      <div className="menu-list">
        {items.some(item=>item.setupChoice==="important"&&item.costState==="recipe_not_configured")&&<section className="ingredient-setup-queue" aria-labelledby="ingredient-queue-heading"><h2 id="ingredient-queue-heading">Ingredient setup</h2><p>Important items stay here until ingredients are added.</p><div className="menu-cards">{items.filter(item=>item.setupChoice==="important"&&item.costState==="recipe_not_configured").map(item=><article className="menu-card" key={`queue-${item.id}`}><h3>{item.name}</h3><div className="card-actions"><button className="file-button" type="button" onClick={()=>setCostingItem(item)}>Add ingredients</button><button className="text-button" type="button" disabled={choiceSavingId===item.id} onClick={()=>void chooseSetup(item,"later")}>Later</button></div></article>)}</div></section>}
...
function InvoiceWorkspace({ restaurant, request, active }: { restaurant: Restaurant; request: <T>(path: string, init?: RequestInit) => Promise<T>; active: boolean }) {
  ...
  if (reviewId) return <ReviewInvoice invoiceId={reviewId} request={request} onBack={() => { setReviewId(""); loadInvoices(); }} onApproved={(changes) => { const approvedId=reviewId; setReviewId(""); setApprovedPriceChanges(changes); setPurchaseId(approvedId); }} onViewOriginal={() => { const invoice=invoices.find((item)=>item.id===reviewId); if(invoice) void openOriginal(invoice); }} />;
  ...
  if (purchaseId) return <ConnectPurchases invoiceId={purchaseId} request={request} onBack={() => { setPurchaseId(""); setNotice(approvedPriceChanges?.length ? `Invoice saved. ${approvedPriceChanges.length} ${approvedPriceChanges.length===1?"price":"prices"} changed.` : "Invoice saved."); setApprovedPriceChanges(null); loadInvoices(); }} />;
...
function PriceChanges({ invoiceId, request, initialChanges, onBack }: { invoiceId:string; request:<T>(path:string, init?:RequestInit)=>Promise<T>; initialChanges:PriceChange[]|null; onBack:()=>void }) {
  ...
  async function markReviewed(id:string){setReviewing(id);setError("");try{await request(`/v1/invoices/${invoiceId}/price-findings/${id}/reviewed`,{method:"PUT",body:"{}"});setChanges(value=>value?.map(change=>change.id===id?{...change,status:"reviewed"}:change)??value);}catch(reason){setError(reason instanceof Error?reason.message:"Price finding couldn't be updated.");}finally{setReviewing("")}}
  return <section className="review-shell price-change-shell" aria-labelledby="price-change-heading"><button className="text-button" type="button" onClick={onBack}>Back to invoices</button><p className="section-code">Cost check</p><h1 id="price-change-heading">Price changes</h1>{initialChanges&&<p className="success-notice" role="status">Invoice approved. {initialChanges.length} {initialChanges.length===1?"price":"prices"} changed.</p>}<p>Compared with the last approved invoice from the same supplier.</p>{error&&<p className="form-error" role="alert">{error}</p>}{changes===null?<p role="status">Checking approved purchases…</p>:changes.length===0?<div className="price-empty"><h2>No price changes to flag.</h2><p>Prices are either within 5% of the last comparable purchase or there is not enough matching history yet.</p></div>:<div className="price-change-list">{changes.map((change)=>{const percentage=Number(change.percentageChange);const increased=percentage>0;return <article className="price-change-card" key={change.id}><p className="invoice-status">{change.status==="reviewed"?"Reviewed":increased?"Cost increase":"Cost decrease"}</p><h2>{change.description} is {increased?"up":"down"} {Math.abs(percentage).toFixed(1)}%</h2><p className="price-comparison"><strong>{formatMoney(change.currentUnitPrice,change.currency)}</strong><span>was {formatMoney(change.previousUnitPrice,change.currency)}{change.unit?` per ${change.unit}`:""}</span></p><p>Last comparable purchase: {formatDate(change.previousInvoiceDate)}.</p>{change.status==="open"&&<button className="file-button" type="button" disabled={reviewing===change.id} onClick={()=>void markReviewed(change.id)}>{reviewing===change.id?"Saving…":"Mark reviewed"}</button>}</article>})}</div>}</section>;
}
...
function ConnectPurchases({ invoiceId, request, onBack }: { invoiceId:string; request:<T>(path:string, init?:RequestInit)=>Promise<T>; onBack:()=>void }) {
  const [response,setResponse]=useState<PurchaseResponse|null>(null); const [decisions,setDecisions]=useState<Record<string,PurchaseDecision>>({}); const [error,setError]=useState(""); const [saving,setSaving]=useState(false); const [showSaved,setShowSaved]=useState(false);
  useEffect(()=>{let cancelled=false;const controller=new AbortController();void request<PurchaseResponse>(`/v1/invoices/${invoiceId}/purchase-review`,{signal:controller.signal}).then(async value=>{if(cancelled)return;if(value.status==="recorded"){setResponse(value);return}const next=suggestedPurchaseDecisions(value);setDecisions(next);const complete=value.lines.length>0&&value.lines.every(line=>validPurchaseDecision(next[line.id],line));if(!complete){setResponse(value);return}setSaving(true);try{const recorded=await request<PurchaseResponse>(`/v1/invoices/${invoiceId}/purchase-receipt`,{method:"PUT",body:JSON.stringify({resolutions:purchaseResolutions(value,next)}),signal:controller.signal});if(!cancelled)setResponse(recorded);}catch(reason){if(!cancelled){setResponse(value);setError(reason instanceof Error?reason.message:"The receipt couldn't be recorded.");}}finally{if(!cancelled)setSaving(false)}}).catch((reason:unknown)=>{if(!cancelled)setError(reason instanceof Error?reason.message:"Purchases couldn't load. Try again.")});return()=>{cancelled=true;controller.abort()};},[invoiceId,request]);
  ...
  async function submit(event:FormEvent){event.preventDefault();if(!response||response.status!=="pending")return;setError("");const invalid=response.lines.some(line=>!validPurchaseDecision(decisions[line.id],line));if(invalid){setError("Finish each line.");return}setSaving(true);try{const next=await request<PurchaseResponse>(`/v1/invoices/${invoiceId}/purchase-receipt`,{method:"PUT",body:JSON.stringify({resolutions:purchaseResolutions(response,decisions)})});setResponse(next);window.scrollTo({top:0});}catch(reason){setError(reason instanceof Error?reason.message:"The receipt couldn't be recorded.");}finally{setSaving(false)}}
  if(!response)return <section className="review-shell"><button className="text-button" type="button" disabled={saving} onClick={onBack}>Back to invoices</button><p role={error?"alert":"status"}>{error||(saving?"Saving invoice…":"Opening purchases…")}</p></section>;
  if(response.status==="recorded")return <ReceiptSummary receipt={response.receipt} replay={response.alreadyRecorded} onBack={onBack}/>;
  const savedCount=response.lines.filter(line=>isSuggestedPurchaseDecision(decisions[line.id])).length;
  const visibleLines=response.lines.filter(line=>showSaved||!isSuggestedPurchaseDecision(decisions[line.id]));
  return <section className="review-shell purchase-shell" aria-labelledby="purchase-heading"><button className="text-button" type="button" onClick={onBack}>Back to invoices</button><p className="section-code">Invoice review</p><h1 id="purchase-heading">Review purchases</h1><div className="purchase-invoice-meta"><strong>{response.invoice.supplierName}</strong><span>{formatDate(response.invoice.invoiceDate)} · {response.invoice.invoiceNumber||"No invoice number"}</span></div><form className="purchase-form" onSubmit={submit}>{savedCount>0&&<div className="saved-match"><strong>{savedCount} saved {savedCount===1?"choice":"choices"}</strong><button className="text-button" type="button" onClick={()=>setShowSaved(value=>!value)}>{showSaved?"Hide":"Review"}</button></div>}{visibleLines.map(line=>{const decision=decisions[line.id]??{action:""};const suggested="suggested" in decision&&decision.suggested;const selectedItem=decision.action==="match"?response.inventoryItems.find(item=>item.id===decision.inventoryItemId):null;const countUnit=decision.action==="create"?decision.countUnit:selectedItem?.countUnit;return <fieldset className="purchase-line" key={line.id}><legend>{line.description}</legend><p className="purchase-source">{line.quantity??"No quantity"} {line.unit??"No purchase unit"}{line.lineTotal?` · ${formatMoney(line.lineTotal,response.invoice.currency)}`:""}</p>{suggested&&<p className="saved-match" role="status">Saved choice</p>}{!line.canTrack&&<p className="review-warning">Quantity or unit missing.</p>}<div className="purchase-options"><label><input type="radio" name={`action-${line.id}`} checked={decision.action==="match"} disabled={!line.canTrack||response.inventoryItems.length===0} onChange={()=>choose(line,"match")}/> Match inventory</label><label><input type="radio" name={`action-${line.id}`} checked={decision.action==="create"} disabled={!line.canTrack} onChange={()=>choose(line,"create")}/> Create item</label><label><input type="radio" name={`action-${line.id}`} checked={decision.action==="ignore"} onChange={()=>choose(line,"ignore")}/> Don’t track</label></div>{decision.action==="match"&&<div className="purchase-fields"><label>Inventory item<select value={decision.inventoryItemId} onChange={event=>{const item=response.inventoryItems.find(candidate=>candidate.id===event.target.value);update(line.id,{inventoryItemId:event.target.value,expectedCountUnit:item?.countUnit??""})}}>{response.inventoryItems.map(item=><option key={item.id} value={item.id}>{item.name} · {item.countUnit}</option>)}</select></label><ConversionField line={line} countUnit={countUnit??"count units"} value={decision.conversion} onChange={value=>update(line.id,{conversion:value})}/></div>}{decision.action==="create"&&<div className="purchase-fields"><label>Item name<input maxLength={50} value={decision.name} onChange={event=>update(line.id,{name:event.target.value})}/></label><label>Category <span>Optional</span><input maxLength={20} value={decision.category} onChange={event=>update(line.id,{category:event.target.value})}/></label><label>Count unit<select value={decision.countUnit} onChange={event=>update(line.id,{countUnit:event.target.value})}>{inventoryUnits.map(unit=><option key={unit}>{unit}</option>)}</select></label><ConversionField line={line} countUnit={decision.countUnit} value={decision.conversion} onChange={value=>update(line.id,{conversion:value})}/></div>}</fieldset>})}{error&&<p className="form-error" role="alert">{error}</p>}<div className="purchase-submit"><button className="file-button" type="button" disabled={saving} onClick={onBack}>Back</button><button className="ledger-button" disabled={saving}>{saving?"Saving…":"Finish invoice"}</button></div></form></section>;
}

function suggestedPurchaseDecisions(response:Extract<PurchaseResponse,{status:"pending"}>):Record<string,PurchaseDecision>{return Object.fromEntries(response.lines.map(line=>{if(line.suggestedAction==="ignore")return [line.id,{action:"ignore",suggested:true}];const item=response.inventoryItems.find(candidate=>candidate.id===line.suggestedInventoryItemId);return [line.id,line.suggestedAction==="match"&&item&&line.suggestedConversion?{action:"match",inventoryItemId:item.id,expectedCountUnit:item.countUnit,conversion:line.suggestedConversion,suggested:true}:{action:""}]}))}
function isSuggestedPurchaseDecision(decision:PurchaseDecision|undefined){return Boolean(decision&&"suggested" in decision&&decision.suggested)}
function purchaseResolutions(response:Extract<PurchaseResponse,{status:"pending"}>,decisions:Record<string,PurchaseDecision>){const resolutions:Record<string,string>[]=[];response.lines.forEach(line=>{const decision=decisions[line.id];if(isSuggestedPurchaseDecision(decision)||!decision?.action)return;if(decision.action==="match"){resolutions.push({lineId:line.id,action:decision.action,inventoryItemId:decision.inventoryItemId,expectedCountUnit:decision.expectedCountUnit,conversion:decision.conversion});return}if(decision.action==="ignore"){resolutions.push({lineId:line.id,action:decision.action});return}resolutions.push({lineId:line.id,action:decision.action,name:decision.name,category:decision.category,countUnit:decision.countUnit,conversion:decision.conversion})});return resolutions}
